...
class Finger:
    def __init__(self, m, k, node: Union[RemoteNode, None] = None):
...
class Node:
    ...
    @property.setter
    def succesor(self, remote_node: RemoteNode):
        pass
    ...
    @property.setter
    def predecessor(self, remote_node: RemoteNode):
        pass
    ...
    def init_finger_table(self, node):
        self.fingerTable[1].node = node.find_successor(
            self.fingerTable[1].start)
        ...
        for i in range(1, m-1):
            if self.fingerTable[i+1].start in (self, self.fingerTable[i].node):
                ...
                self.fingerTable[i +
                                 1].node = node.findSuccessor(self.fingerTable[i+1].start)
